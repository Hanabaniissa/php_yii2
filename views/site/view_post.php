...
<div style="margin-top: 50px">
    ...
    <?php if (!Yii::$app->user->isGuest && Yii::$app->user->id == $onePost->created_by): ?>
        <div class="row">
            <div class="col-lg-6">
                <?= \yii\helpers\Html::a('Update', \yii\helpers\Url::to(['/site/post', 'postId' => $onePost->id]), ['class' => 'btn btn-warning btn-lg', 'style' => 'margin-right: 3px; color: #47555e']) ?>

                <?= \yii\helpers\Html::a('Delete', \yii\helpers\Url::to(['/site/delete-post', 'postId' => $onePost->id]), ['class' => 'btn btn-danger btn-lg', 'onclick' => 'ConfirmDelete()']) ?>
            </div>
        </div>
    <?php endif; ?>
</div>
